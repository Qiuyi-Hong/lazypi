...
The Settings tab (also available before first-run setup) toggles **Auto-check updates at startup** (off by default; async npm checks notify when available) and **Show Community tab** (on by default). These preferences live in `<agent-dir>/lazypi.json`, not Pi settings. Manual Community and Updates shortcuts still work when the Community tab is hidden; the startup check does not block the UI and registry failures are silent.
